...
docker stop 392acbcbdaa0 # container name or container id
docker start 392acbcbdaa0 # start docker container again
docker rm 392acbcbdaa0 # remove in runnable list (after stop)
docker rmi 392acbcbdaa0 # remove container
```
...
## Practice 3. Pytorch Nvidia Docker Container with file exchange

- pull 23.04-py3 pytorch nvidia docker container
...
### Practice 4. Combine Python and Nvidia Docker Container
...
- Install Nvidia Docker Container including Pytorch
...
```bash
docker run -it -v /ML_host_directory/:/ML_container_directory/ --ipc=host --gpus all --name ML_container_Pytorch nvcr.io/nvidia/pytorch:23.04-py3

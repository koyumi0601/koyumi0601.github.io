---
layout: single
title: "Numpy-Torch Basic"
categories: machinelearning
tags: [ML, Machine Learning, AI, Pytorch]
toc: true
...
print(a.dtype) # int64
print(a.shape) # size([4])
# b = np.array([1, 2, 3.1, 4])
b = torch.tensor([1, 2, 3.1, 4])
print(b.dtype) # 하나라도 실수면 자동으로 실수 타입, float32
```

- shape, ndim, size, numel
```python
A = np.array([[1, 2],[3, 4]]) 
...
- zeros, zeros_like, ones, arange, linspace
```python
...
print(A[:][2]) # [7, 8, 9] 위와 동일한 표현은 아니다
```
...
- numpy의 여러 함수들, random
```python
A = np.random.randn(3, 3) # -1~1, 평균 0 분산 1 normal distribution, gaussian
B = np.random.rand(3, 3) # 0~1, uniform
print(A)
print(B)
print(A[A[:, 0] < 0, :]) # 0번째 열이 음수인 경우, 모든 열
```

- pytorch의 여러 함수들, random
```python
A = torch.randn(3, 3) # -1~1, 평균 0 분산 1 normal distribution, gaussian
B = torch.rand(3, 3) # 0~1, uniform
print(A)
print(B)

    ...
    , ocr_api(nullptr)
{
    ...
    {
        arg1 = "stream";
        structdata.toolModeName = "stream";
    }
    else
    {
        arg1 = arguments[1].toStdString();
        structdata.toolModeName = "emulate";
    }
...
bool MainWindow::eventFilter(QObject* obj, QEvent* event)
{
    ...
    if (obj == ui->mainGraphicsView->viewport() && structdata.mouseButtonStatus == Qt::LeftButton)
    {
        ...
        width = structdata.ocrdata.positionX2 - structdata.ocrdata.positionX1 + 1;
        height = structdata.ocrdata.positionY2 - structdata.ocrdata.positionY1 + 1;
        if (structdata.ocrdata.positionX1 > structdata.ocrdata.positionX2)
        {
            x = structdata.ocrdata.positionX2;
            width = structdata.ocrdata.positionX1 - structdata.ocrdata.positionX2 + 1;
        }
        if (structdata.ocrdata.positionY1 > structdata.ocrdata.positionY2)
        {
            y = structdata.ocrdata.positionY2;
            height = structdata.ocrdata.positionY1 - structdata.ocrdata.positionY2 + 1;
        }
...
//    /// mouse events for stencil1
//    else if (obj == ui->mainGraphicsView->viewport() && structdata.mouseButtonStatus == Qt::RightButton)
//    {
//        int candidatePositionX0 = mouseEvent->pos().x() - stencilRoiRectHalfSize;
//        int candidatePositionY0 = mouseEvent->pos().y() - stencilRoiRectHalfSize;
//        int candidatePositionX1 = candidatePositionX0 + stencilRoiRectSize;
//        int candidatePositionY1 = candidatePositionY0 + stencilRoiRectSize;
//        if (event->type() == QEvent::MouseButtonPress)
//        {
...
//            }
//            // check whether it is in inside of mainwindow with a certain margin
//            if (((0 <= candidatePositionX0) && (candidatePositionX1 <= fixedMainWindowWidth-1)) &&
//                    ((0 <= candidatePositionY0) && (candidatePositionY1 <= fixedMainWindowHeight-1)))
//            {
//                qDebug() << candidatePositionX0 << ", " << candidatePositionY0;
//                qDebug() << candidatePositionX1 << ", " << candidatePositionY1;
//                qDebug() << "\n";
//            }
//        }
//    }
    ...
    else if (obj == ui->mainGraphicsView->viewport() && structdata.mouseButtonStatus == Qt::RightButton)
    {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        for (int dim3 = 0; dim3 < dim3Size; ++dim3)
                        {
                            ...
                            {
                                if (
                                        // whether already existed rubberband point1 is included in current canditate
        ...
        width = structdata.stencildata.positionA[0][1][structdata.stencildata.updateTargetIdx] - structdata.stencildata.positionA[0][0][structdata.stencildata.updateTargetIdx] + 1;
        height = structdata.stencildata.positionA[1][1][structdata.stencildata.updateTargetIdx] - structdata.stencildata.positionA[1][0][structdata.stencildata.updateTargetIdx] + 1;
        if (structdata.stencildata.positionA[0][0][structdata.stencildata.updateTargetIdx] > structdata.stencildata.positionA[0][1][structdata.stencildata.updateTargetIdx])
        {
            ...
            width = structdata.stencildata.positionA[0][0][structdata.stencildata.updateTargetIdx] - structdata.stencildata.positionA[0][1][structdata.stencildata.updateTargetIdx] + 1;
        }
        if (structdata.stencildata.positionA[1][0][structdata.stencildata.updateTargetIdx] > structdata.stencildata.positionA[1][1][structdata.stencildata.updateTargetIdx])
        {
            ...
            height = structdata.stencildata.positionA[1][0][structdata.stencildata.updateTargetIdx] - structdata.stencildata.positionA[1][1][structdata.stencildata.updateTargetIdx] + 1;
        }
...
void MainWindow::onOcrRoiPushButtonClicked()
{
    if (structdata.ocrdata.isRoiCandidated)
    {
        ...
        structdata.ocrdata.numEqaulFrameCount = numSteadyStateFrame;
    }
}
...
void MainWindow::onGenerateStencilRoiPushButtonClicked()
{
    ...
    if (structdata.stencildata.isRoiDetermined)
    {
        ...
        if (structdata.probeType == "Convex")
        {
            // calc apex pont
            QPointF apexPoint;
//            if ((topLeftImgX == topRightImgX) && (topRightImgX == bottomRightImgX))  // actually convex
//            {
                apexPoint = Utils::intersectionPoint(topLeftPoint, bottomLeftPoint, topRightPoint, bottomRightPoint);
//            }
//            else // actually linear
//            {
//                apexPoint = QPointF(((float)topLeftImgX + (float)topRightImgX) * 0.5f, -10000000.0f);
//            }
            qDebug() << apexPoint;
...
//    auto start_time = std::chrono::high_resolution_clock::now();
    ...
    if (structdata.ocrdata.isRoiDetermined)
    {
        ...
        {
            ++structdata.ocrdata.numEqaulFrameCount;
        }
        else
            ...
            {
                structdata.ocrdata.numEqaulFrameCount = 0;
            }

        if (structdata.ocrdata.numEqaulFrameCount >= numSteadyStateFrame)
        {

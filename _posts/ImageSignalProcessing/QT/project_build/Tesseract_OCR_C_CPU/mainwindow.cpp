...
void MainWindow::connectCallbacks()
{
    ...
    connect(ui->deleteItemPushButton, &QPushButton::clicked, this, &MainWindow::onDeleteItemPushButtonClicked);
    // combobox
    ...
    // tableWidget
    connect(ui->monitoringTableWidget, &QTableWidget::cellClicked, this, &MainWindow::onTableWidgetListClicked);
}
...
void MainWindow::initPrivateVariables()
{
    redrawingState = "live";   // "live" or "review"
    selectedTableIndex = "";
}
...
bool MainWindow::eventFilter(QObject* obj, QEvent* event)
{
    ...
    if (obj == ui->mainGraphicsView->viewport())
    {
        ...
        else if (structdata.mouseButtonStatus == Qt::RightButton) // Stencil
        {
            ...
            //rightMouseButtonEvent_NEW(mouseEvent);
        }
...
void MainWindow::rightMouseButtonEvent_NEW(QMouseEvent* mouseEvent)
{
    if (mouseEvent->type() == QEvent::MouseButtonPress)
    {
        getMouseMoveStartPos(mouseEvent->pos());
    }
    else if (mouseEvent->type() == QEvent::MouseMove)
    {
        moveStencilBox(mouseEvent->pos());
    }
    else if (mouseEvent->type() == QEvent::MouseButtonRelease)
    {
        generateStencilBox(mouseEvent->pos());
    }
    printStencilData(false);
}


void MainWindow::getMouseMoveStartPos(const QPoint& pos)
{
    ...
    {
        prevMouseMovePosX = pos.x();
        prevMouseMovePosY = pos.y();
        isMouseMoveState = false;
    }
}

void MainWindow::generateStencilBox(const QPoint& pos)
{
    int startpos_x = pos.x() - stencilRoiRectHalfSize;
    int startpos_y = pos.y() - stencilRoiRectHalfSize;
    int endpos_x = pos.x() + stencilRoiRectHalfSize;
    int endpos_y = pos.y() + stencilRoiRectHalfSize;
    bool isOverlapped = searchOverlapedRubberBand(startpos_x, startpos_y, endpos_x, endpos_y);
    structdata.stencildata.updateTargetIdx = searchAlreadyExistStencilRubberBandIndex(pos);
    if ((structdata.stencildata.updateTargetIdx == -1) && (Utils::andArray(structdata.stencildata.isRoiCandidated) == false) && isOverlapped == false)
    {
        structdata.stencildata.updateTargetIdx = Utils::findFirstFalse(structdata.stencildata.isRoiCandidated);
        structdata.stencildata.positionA[0][0][structdata.stencildata.updateTargetIdx] = startpos_x;
        structdata.stencildata.positionA[1][0][structdata.stencildata.updateTargetIdx] = startpos_y;
        structdata.stencildata.positionA[0][1][structdata.stencildata.updateTargetIdx] = endpos_x;
        structdata.stencildata.positionA[1][1][structdata.stencildata.updateTargetIdx] = endpos_y;
        structdata.stencildata.isRoiCandidated[structdata.stencildata.updateTargetIdx] = true;
        structdata.stencildata.rectInfo[structdata.stencildata.updateTargetIdx].x = structdata.stencildata.positionA[0][0][structdata.stencildata.updateTargetIdx];
        structdata.stencildata.rectInfo[structdata.stencildata.updateTargetIdx].y = structdata.stencildata.positionA[1][0][structdata.stencildata.updateTargetIdx];
        structdata.stencildata.rectInfo[structdata.stencildata.updateTargetIdx].width = stencilRoiRectSize;
        structdata.stencildata.rectInfo[structdata.stencildata.updateTargetIdx].height = stencilRoiRectSize;
    }
    else if (structdata.stencildata.updateTargetIdx != -1)
    {
        if (isMouseMoveState)
        {
            structdata.stencildata.isRoiCandidated[structdata.stencildata.updateTargetIdx] = true;
...
void MainWindow::moveStencilBox(const QPoint& pos){
    if (structdata.stencildata.updateTargetIdx != -1)
    {
        int curMouseMovePosX = pos.x();
        int curMouseMovePosY = pos.y();
        int deltaPosX = prevMouseMovePosX - curMouseMovePosX;
        int deltaPosY = prevMouseMovePosY - curMouseMovePosY;
        int candidateStartpos_x = structdata.stencildata.positionA[0][0][structdata.stencildata.updateTargetIdx] - deltaPosX;
        int candidateStartpos_y = structdata.stencildata.positionA[1][0][structdata.stencildata.updateTargetIdx] - deltaPosY;
        int candidateEndpos_x = structdata.stencildata.positionA[0][1][structdata.stencildata.updateTargetIdx] - deltaPosX;
        int candidateEndpos_y = structdata.stencildata.positionA[1][1][structdata.stencildata.updateTargetIdx] - deltaPosY;
        bool isOverlapped = searchOverlapedRubberBand(candidateStartpos_x, candidateStartpos_y, candidateEndpos_x, candidateEndpos_y);
        if (isOverlapped == false)
        {
            structdata.stencildata.positionA[0][0][structdata.stencildata.updateTargetIdx] -= deltaPosX;
            structdata.stencildata.positionA[1][0][structdata.stencildata.updateTargetIdx] -= deltaPosY;
            structdata.stencildata.positionA[0][1][structdata.stencildata.updateTargetIdx] -= deltaPosX;
            structdata.stencildata.positionA[1][1][structdata.stencildata.updateTargetIdx] -= deltaPosY;
            structdata.stencildata.rectInfo[structdata.stencildata.updateTargetIdx].x = structdata.stencildata.positionA[0][0][structdata.stencildata.updateTargetIdx];
            structdata.stencildata.rectInfo[structdata.stencildata.updateTargetIdx].y = structdata.stencildata.positionA[1][0][structdata.stencildata.updateTargetIdx];
        }
        structdata.stencildata.isRoiCandidated[structdata.stencildata.updateTargetIdx] = true;
        ...
        prevMouseMovePosX = curMouseMovePosX;
        prevMouseMovePosY = curMouseMovePosY;
        isMouseMoveState = true;
    }
}
...
void MainWindow::onOcrRoiPushButtonClicked()
{
    ...
    {
        bool isValidOcrRoi = true;
        int candidateRecoveredX = (int)((float)structdata.ocrdata.rectInfo.x * structdata.widthFactor + 0.5f);
        int candidateRecoveredY = (int)((float)structdata.ocrdata.rectInfo.y * structdata.heightFactor + 0.5f);
        int candidateRecoveredWidth = (int)((float)structdata.ocrdata.rectInfo.width * structdata.widthFactor + 0.5f);
        int candidateRecoveredHeight = (int)((float)structdata.ocrdata.rectInfo.height * structdata.heightFactor + 0.5f);
        for (int row = 0; row < ui->monitoringTableWidget->rowCount(); ++row)
        {
            std::string tableDataIndex = ui->monitoringTableWidget->item(row, 0)->text().toStdString() + "-" + ui->monitoringTableWidget->item(row, 1)->text().toStdString() + "-" + ui->monitoringTableWidget->item(row, 2)->text().toStdString();
            cv::Mat tmpOcrCvImg = reviewData[tableDataIndex].orgImg(cv::Rect(candidateRecoveredX, candidateRecoveredY, candidateRecoveredWidth, candidateRecoveredHeight));
            std::string tmpOcrOutText = runOcr(tmpOcrCvImg);
            std::string storedDepth = ui->monitoringTableWidget->item(row, 2)->text().toStdString();
            if (tmpOcrOutText != storedDepth)
            {
                isValidOcrRoi = false;
            }
        }
        if (isValidOcrRoi == true)
        {
            // update review data
            for (int row = 0; row < ui->monitoringTableWidget->rowCount(); ++row)
            {
                std::string tableDataIndex = ui->monitoringTableWidget->item(row, 0)->text().toStdString() + "-" + ui->monitoringTableWidget->item(row, 1)->text().toStdString() + "-" + ui->monitoringTableWidget->item(row, 2)->text().toStdString();
                reviewData[tableDataIndex].ocrdata.recoveredRectInfo = cv::Rect(candidateRecoveredX, candidateRecoveredY, candidateRecoveredWidth, candidateRecoveredHeight);
                reviewData[tableDataIndex].ocrdata.rectInfo = structdata.ocrdata.rectInfo;
            }
            liveData.ocrdata.recoveredRectInfo = cv::Rect(candidateRecoveredX, candidateRecoveredY, candidateRecoveredWidth, candidateRecoveredHeight);
            liveData.ocrdata.rectInfo = structdata.ocrdata.rectInfo;
            // update live data
...
void MainWindow::onConfirmStencilRoiPushButtonClicked()
{
    if (Utils::andArray(structdata.stencildata.isRoiCandidated))
    {
        ...
        std::vector<int> cornerIndices = findCornerPoints(structdata.stencildata.rectInfo);
        for (int idx : cornerIndices)
        {
            if (idx == -1)
            {
                structdata.stencildata.isRoiDetermined = false;
                QMessageBox::warning(nullptr, "Warning", "Invalid ROI selected.");
                break;
            }
        }
    }
...
void MainWindow::onGenerateStencilRoiPushButtonClicked()
{
    ...
    {
        std::vector<int> cornerIndices = findCornerPoints(structdata.stencildata.rectInfo);
        int topLeftIdx = cornerIndices[0];
        int topRightIdx = cornerIndices[1];
        int bottomLeftIdx = cornerIndices[2];
        int bottomRightIdx = cornerIndices[3];
...
std::vector<int> MainWindow::findCornerPoints(const std::vector<cv::Rect>& rectInfo)
{
    std::vector<int> cornerIndices(4, -1);
    int topLeftIdx = -1, topRightIdx = -1, bottomLeftIdx = -1, bottomRightIdx = -1;
    std::vector<float> xA = {};
    std::vector<float> yA = {};
    std::vector<bool> isLeft = {false, false, false, false};
    std::vector<bool> isTop = {false, false, false, false};

    for (int dim3 = 0; dim3 < dim3Size; ++dim3)
    {
        xA.push_back(float(rectInfo[dim3].x) + float(rectInfo[dim3].width) / 2.0f);
        yA.push_back(float(rectInfo[dim3].y) + float(rectInfo[dim3].height) / 2.0f);
    }

    auto Left2Indices = Utils::find2SmallestIndices(xA);
    auto Top2Indices = Utils::find2SmallestIndices(yA);

    isLeft[Left2Indices.first] = true;
    isLeft[Left2Indices.second] = true;
    isTop[Top2Indices.first] = true;
    isTop[Top2Indices.second] = true;

    for (int dim3 = 0; dim3 < dim3Size; ++dim3)
    {
        if ((isTop[dim3] == true) && (isLeft[dim3] == true))
        {
            topLeftIdx = dim3;
        }
        else if ((isTop[dim3] == true) && (isLeft[dim3] == false))
        {
            topRightIdx = dim3;
        }
        else if ((isTop[dim3] == false) && (isLeft[dim3] == true))
        {
            bottomLeftIdx = dim3;
        }
        else if ((isTop[dim3] == false) && (isLeft[dim3] == false))
        {
            bottomRightIdx = dim3;
        }
    }
    cornerIndices[0] = topLeftIdx;
    cornerIndices[1] = topRightIdx;
    cornerIndices[2] = bottomLeftIdx;
    cornerIndices[3] = bottomRightIdx;
...
{
    bool isTriValueReady = (!structdata.systemName.empty()) && (!structdata.probeName.empty()) && (!structdata.ocrdata.depth.empty());
    bool isTwoRoisReady = (structdata.stencildata.isMaskExist && structdata.ocrdata.isRoiDetermined);
    bool isReadyToPushItem = (isTriValueReady && isTwoRoisReady);
    if (isReadyToPushItem)
    {
        ...
        // search target update index of tableWidget
        for (int row = 0; row < ui->monitoringTableWidget->rowCount(); ++row)
        {
        ...
        // update tableWidget and tableData
        if (reply == QMessageBox::Yes)
        {
            // update string of tableWidget
            ...
            // update tableData
            std::string tableDataIndex = structdata.systemName + "-" + structdata.probeName + "-" + structdata.ocrdata.depth;
            reviewData[tableDataIndex] = structdata;
            // update color of tableWidget
...
void MainWindow::onDeleteItemPushButtonClicked()
{
    ...
    if (!selectedItems.isEmpty())
    {
        ...
        reviewData.erase(selectedTableIndex);
        selectedTableIndex = "";
        redrawingState = "live";
    }
}
...
/// tableWidget interactions
void MainWindow::onTableWidgetListClicked(int row, int column)
{
    ...
    if (item)
    {
        ...
        if (item -> isSelected())
        {
            ...
            // transit review -> live
            if (redrawingState == "review")
            {
                structdata = liveData;      // restore state of live
                selectedTableIndex = "";
                redrawingState = "live";
            }
        }
        else
        {
            ...
            // transit live -> review
            if (redrawingState == "live")
            {
                liveData = structdata;      // backup current state of live
            }
            selectedTableIndex = tableDataIndex;
            structdata = reviewData[selectedTableIndex];
            redrawingState = "review";
        }
    }
}
...
{
    // reload image according to gui state
    cv::Mat cvImage;
    if (redrawingState == "live")
    {
        cvImage = reloadCvImage();
    }
    else if (redrawingState == "review")
    {
        cvImage = structdata.scaledOrgImg;
    }

    // draw image on scene
    if (structdata.stencildata.isMaskExist && ui->applyStencilCheckBox->isChecked())
    {
...
cv::Mat MainWindow::reloadCvImage() {
    ...
    structdata.orgImg = cvOrgImage.clone();
    structdata.scaledOrgImg = cvScaledImage.clone();
...
void MainWindow::redrawScene(const cv::Mat& image)
{
    ...
    if (redrawingState == "review")
    {
        QString tmpText = "In review mode";
        int fontSize = 20;
        int textLocationCol = 10;
        int textLocationRow = 10;
        QPainter tmpPainter(&qtImage);
        tmpPainter.setPen(Qt::white);
        tmpPainter.setFont(QFont("Arial", fontSize));
        QFontMetrics tmpFm(tmpPainter.font());
        int tmpTextWidth = tmpFm.horizontalAdvance(tmpText);
        int tmpTextHeight = tmpFm.height();
        QRect tmpRect(textLocationCol, textLocationRow, tmpTextWidth, tmpTextHeight);
        tmpPainter.fillRect(tmpRect, Qt::black);
        tmpPainter.drawText(tmpRect, Qt::AlignCenter, tmpText);
    }
    pixmapItem = new QGraphicsPixmapItem(QPixmap::fromImage(qtImage));
